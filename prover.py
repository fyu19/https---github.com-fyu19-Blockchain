...
def gen_merkle_proof(leaves, pos):
    ...
    for level in range(height):
        ...
        # Merkle proof
        #If the position of the data that is been checked is on the left, then the hash of the right one is in the proof,
        #Since it is a binary tree, the right one always exist
        #The other case is symetrical
        if level_pos%2 == 0:                         
            path.append(state[level_pos+1])        
        else :                                     
            path.append(state[level_pos-1])
        #Generation of the hash of the level above
        new_state = []                              #
        for i in range(0,len(state),2):
            new_state.append(hash_internal_node(state[i],state[i+1]))
        #Assignement to the new state and the new position of the data that is been checked
        state = new_state                           
...
if __name__ == "__main__":
    ...
    # write proof to file
    write_proof("C:/Users\YU François\OneDrive\Bureau\Taiwan\Blockchain\sample\sample\proof.txt", proof)
